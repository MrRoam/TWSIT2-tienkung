...
EXPTID=${1:-tk_priv_cc_$(date +%m%d_%H%M)}
DEVICE=${2:-cuda:0}
NUM_ENVS=${3:-256}
MAX_ITER=${4:-10000}
...
if [ -z "${WANDB_API_KEY:-}" ]; then
  echo "[train_tienkung_priv_mimic_cc] WANDB_API_KEY is not set, append --no_wandb."
  EXTRA_ARGS+=("--no_wandb")
fi
...
/home/qsh/miniconda3/envs/twist2/bin/python train.py \
  --task tienkung_priv_mimic_cc \
  --proj_name tienkung_priv_mimic_cc \
  --exptid "${EXPTID}" \
  --device "${DEVICE}" \
  --num_envs "${NUM_ENVS}" \
