...
EXPTID=${1:-tk_stufut_cc_stage1_$(date +%m%d_%H%M)}
DEVICE=${2:-cuda:0}
NUM_ENVS=${3:-256}
MAX_ITER=${4:-10000}
...
if [ -z "${WANDB_API_KEY:-}" ]; then
  echo "[train_tienkung_stu_future_cc_stage1] WANDB_API_KEY is not set, append --no_wandb."
  EXTRA_ARGS+=("--no_wandb")
fi
...
/home/qsh/miniconda3/envs/twist2/bin/python train.py \
  --task tienkung_stu_future_cc_stage1 \
  --proj_name tienkung_stu_future_cc_stage1 \
  --exptid "${EXPTID}" \
  --teacher_exptid "None" \
