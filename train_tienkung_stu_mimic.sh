...
# Usage:
#   bash train_tienkung_stu_mimic.sh <exptid> <teacher_exptid> <teacher_checkpoint> <device> <num_envs> <max_iterations> <motion_yaml> [extra args...]

EXPTID=${1:-tk_stu_$(date +%m%d_%H%M)}
...
if [ -z "${WANDB_API_KEY:-}" ]; then
  echo "[train_tienkung_stu_mimic] WANDB_API_KEY is not set, append --no_wandb."
  EXTRA_ARGS+=("--no_wandb")
fi
...
/home/qsh/miniconda3/envs/twist2/bin/python train.py \
  --task tienkung_stu_mimic \
  --proj_name tienkung_stu_mimic \
  --exptid "${EXPTID}" \
