...
EXPTID=${1:-tk_sturl_cc_$(date +%m%d_%H%M)}
TEACHER_EXPTID=${2:-tienkung_priv_placeholder}
TEACHER_CHECKPOINT=${3:--1}
DEVICE=${4:-cuda:0}
NUM_ENVS=${5:-256}
MAX_ITER=${6:-10000}
MOTION_YAML=${7:-/home/qsh/workspace_twist2/TWIST2/legged_gym/motion_data_configs/tienkung_ei_train30.yaml}
...
if [ -z "${WANDB_API_KEY:-}" ]; then
  echo "[train_tienkung_stu_rl_cc] WANDB_API_KEY is not set, append --no_wandb."
  EXTRA_ARGS+=("--no_wandb")
fi
...
/home/qsh/miniconda3/envs/twist2/bin/python train.py \
  --task tienkung_stu_rl_cc \
  --proj_name tienkung_stu_rl_cc \
  --exptid "${EXPTID}" \
